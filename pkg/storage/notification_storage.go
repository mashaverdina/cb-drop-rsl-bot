...
func (s *NotificationStorage) DisableNotification(u entities.User, n entities.Notification) error {
	return s.pg.ExecuteInTransaction(hasql.Primary, func(db *gorm.DB) error {
		return db.Exec("delete from notification_users where user_id = ? and fire_id in (select fire_id from notification_fires where notification_fires.notification_id=?)", u.UserID, n.NotificationID).Error
	})
}

func (s *NotificationStorage) EnableNotification(u entities.User, n entities.Notification) error {
	return s.pg.ExecuteInTransaction(hasql.Primary, func(db *gorm.DB) error {
